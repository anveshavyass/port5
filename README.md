...
4. Embeds review themes into Milvus, a dedicated vector database, for semantic search.
...
| Database | **Postgres** (structured columns) **+ Milvus** (vectors) | Postgres stays the source of truth for every structured field; Milvus is a purpose-built ANN vector store with dedicated indexing (HNSW/IVF via AUTOINDEX) that scales better than a `vector` column once the corpus grows well past a demo-sized table. Milvus only stores `id` + `embedding` — search results are hydrated back to full rows via a Postgres lookup by id |
| DB access | **`psycopg2`, plain SQL, parameterized queries** (no ORM) for Postgres; **`pymilvus`** for vector search | At this scale an ORM adds a layer with nothing to abstract, so Postgres access stays plain SQL. Parameterized (`%s` placeholders, never string interpolation) to rule out SQL injection |
...
```
 ...
 ③  POSTGRES  →  reviews table (raw + classification columns)
        │
        ▼
 ④  EMBEDDING (OpenAI) → Milvus collection (id + vector; id maps
                            back to the Postgres row)
        │
   ┌────┴─────────────────────────┐
   ▼                              ▼
 ⑤ AGGREGATION (pandas/SQL)     ⑥ RAG QUERY ENGINE
   ...
   sentiment agreement            SQL filter OR Milvus similarity
...
├── .env.example
├── .gitignore
├── docker-compose.yml                      # Milvus standalone (milvus + etcd + minio)
├── data/
...
│   ├── config.py                           # loads .env
│   ├── db/
│   │   ├── init_db.sql                     # Postgres schema (no embeddings)
│   │   ├── connection.py                   # psycopg2 connection helper
│   │   ├── milvus_client.py                # Milvus collection: id + embedding, nearest-neighbor search
│   │   └── queries.py                      # plain, parameterized SQL: insert, select, aggregate; vector_search hydrates Milvus hits from Postgres
│   ├── scripts/
│   │   └── migrate_embeddings_to_milvus.py # one-off: copy existing embeddings into Milvus
│   ├── pipeline/
...
# backend
cd backend
cp ../.env.example .env        # fill in OPENAI_API_KEY, DATABASE_URL, MILVUS_URI
